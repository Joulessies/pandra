...
export default function SignInScreen() {
  ...
  const { email, password, passwordVisible, focusedField, loading, error } =
    signInStore;
  ...
  const handleSignIn = async (
    ...
    signInStore.setLoading(true);
    signInStore.setError("");

    if (
      (targetEmail.toLowerCase() === ADMIN_CREDENTIALS.email.toLowerCase() ||
        targetEmail.toLowerCase().startsWith("admin@")) &&
      targetPass === ADMIN_CREDENTIALS.password
    ) {
      await loginAsAdmin();
      router.replace("/" as any);
      signInStore.setLoading(false);
      return;
    } else if (
      targetEmail.toLowerCase() === ADMIN_CREDENTIALS.email.toLowerCase() &&
      targetPass !== ADMIN_CREDENTIALS.password
    ) {
      signInStore.setError("Incorrect admin password. (Default is admin123)");
      signInStore.setLoading(false);
      return;
    }
    ...
    try {
      if (isLoaded && signIn) {
        ...
        if (result.status === "complete") {
          ...
          signInStore.setError(
            `Sign in requires additional verification (${result.status}).`,
          );
        }
      }
    } catch (err: any) {
        ...
        >
          ...
          <YStack gap={6} marginTop={16} marginBottom={24}>
            ...
            <Text
              ...
              lineHeight={19}
            >
              Sign in to access your custom AI widgets and dashboard.
            </Text>
          </YStack>
          ...
          <YStack gap={16}>
            ...
            <GoogleSignInButton
              ...
              onError={(msg) => signInStore.setError(msg)}
            />
            ...
            <YStack gap={6}>
              ...
              >
                ...
                <Input
                  ...
                  value={email}
                  onChangeText={signInStore.setEmail}
                  onFocus={() => signInStore.setFocusedField("email")}
                  onBlur={() => signInStore.setFocusedField(null)}
            ...
            <YStack gap={6}>
              ...
              >
                ...
                <Input
                  ...
                  onFocus={() => signInStore.setFocusedField("password")}
                  onBlur={() => signInStore.setFocusedField(null)}
                ...
                <TouchableOpacity
                  ...
                  style={{ padding: 6 }}
                  onPress={() =>
                    signInStore.setPasswordVisible(!passwordVisible)
                  }
                >
                  {passwordVisible ? (
